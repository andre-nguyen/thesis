...
\subsubsection{Étape 2: Détection de contours} La détection de contours se fait au moyen de l'algorithme de Canny \citep{Canny1986} qui a son tour se fait en une série de 4 étapes:
...
\begin{enumerate}
  ...
  \end{align}
  \item Suppression des non-maxima (expliquer AVEC $\theta$)
  \item Un double seuil, l'un haut $T_h$ et l'autre bas $T_b$, est appliqué par hystéresis pour atténuer les pixels de contours trop faibles. Un pixel $G_{ij}$ plus fort que $T_h$ est marqué comme un pixel au contour fort. Les pixels plus faibles que $T_b$ sont supprimés (mise à $0$). Les pixels entre les deux sont gardés seulement s'ils sont adjacents à un pixel fort.
\end{enumerate}
