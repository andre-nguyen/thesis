...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.6\linewidth]{images/ugv_goal}
...
\begin{figure}[ht]
  ...
  \caption{
    Illustration du calcul de la prochaine pose objectif a) dans le cas normal b) dans le cas d'un coin à angle aiguë.
  }
...
\begin{algorithm}[ht]
  ...
  \Input{$\mathcal S$ Les nuages de points sauvegardés par le système de SLAM}
  ...
  Get map from rtabmap\\
    ...
    \ForEach{$p \in \mathcal S$}{
      ...
      Filter ground and ceiling from $p^g$\\
      Insert and raycast into octomap\\
      Also assemble into large cloud
    }\\
    \Indm
  cloudmap_cb\\
    \Indp
  ...
  determine frontiers\\
    \Indp
    \ForEach{leaf in octomap}{
      \If{empty}{
        add leaf to frontier candidates
      }
    }\\
    crop out candidates outside of hull\\
    \ForEach{candidate in frontier candidates}{
      $\vec{n}_i = $ estimate normal of candidate through PCA using voxels in a radius around\\
      \If{$| {\vec{n}_i}_z^g | < \alpha$}{
        Reject voxel if it is too close to the structure
      }\\
    }\\
    \Indm
