...
\begin{algorithm}[ht]
  ...
  \Output{${po}^c$ la position objectif de la caméra dans le repère $\mathsf{C}$}
...
Puisque notre robot est terrestre, la tâche de planification de trajectoire se simplifie en deux dimensions. Pour ce faire, le modèle 3D en construction est projetté sur une grille 2D pour établir une carte d'occupation. On prend un sous ensemble local de la carte de taille $2D \times 2D$ (nous rappellons que $D$ est la distance à la structure désirée) autour de la position de la caméra pour calculer une trajectoire par champ de potentiel \citep{Khatib1990, Choset2005}.

Soit $k$ cellules occupées aux coordonnées $\{x_j\}^k_{j=1}$ nous avons la fonction de potentiel
\begin{align}
  N(x) &= \alpha \| x - {po}^g \|^2 + \sum^k_{j=1} I_j(x)d_j(x)
\end{align}
où $\alpha > 0$. $d_j$ est la contribution au potentiel de répulsion fourni par la $j^{\text{ième}}$ cellule occupée.
\begin{align}
  d_j &= \frac{1}{\beta \| x - x_j \|}
\end{align}
où $\beta > 0$ et $I_j(x)$ est le terme limitant la portée de $d_j$ par
\begin{align}
  I_j(x) =     \begin{cases}
    ...
    \end{cases}
\end{align}
Pour se rendre à la position objectif, is suffit de suivre le gradient négatif du champ de potentiel.
\begin{align}
  - \nabla N(x) = 2\alpha({po}^g-x) + \sum_{i \in J_x} \frac{1}{\beta \| x - x_i \|^3} (x - x_i)
\end{align}
où $J_x$ est l'ensemble des cellules occupées dans un voisinage $D$ de $x$, c'est-à-dire $J_x = \{j : I_j(x) = 1\}$. Considérons maintenant l'espace $\mathcal M_D = \left\lbrace x : J_{x} \neq \emptyset \right\rbrace $, la région à une distance $D$ de la structure.  
